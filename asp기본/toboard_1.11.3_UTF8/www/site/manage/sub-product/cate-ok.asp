...
ElseIf mode="mod" Then 
	...
	Sql = "SELECT c_image FROM tbl_cate WHERE c_code='" & c_code & "'"
	Rs.Open Sql, DbCon, 1
	...
	If UploadForm("c_image").FileName<>"" Then
		...
		c_image = UploadForm("c_image").Save(, False)
		Dbcon.Execute "UPDATE tbl_cate SET c_image='" & Mid(c_image, InstrRev(c_image, "\")+1)  & "' WHERE c_code='" & c_code & "'"
	End If

	Sql = "UPDATE tbl_cate SET "
	Sql = Sql & "c_name=N'" & c_name & "', "
...
ElseIf mode="del" Then 
	...
	Sql = "DELETE FROM tbl_cate WHERE c_code LIKE '" & c_code & "%';"
...
ElseIf mode="up" Then 
	...
	Set Rs = Server.Createobject("ADODB.Recordset")
	Sql = "SELECT TOP 1 c_code, c_lev FROM tbl_cate WHERE c_lev<" & c_lev & ""
	...
	If Rs.EOF=False Then
		tg_code = Rs("c_code")
		tg_lev = Rs("c_lev")
		Sql = "UPDATE tbl_cate SET c_lev='" & c_lev & "' WHERE c_code='" & tg_code & "';" & Chr(13) & Chr(10)
		Sql = Sql & "UPDATE tbl_cate SET c_lev='" & tg_lev & "' WHERE c_code='" & c_code & "';"
		Dbcon.Execute Sql
	End If
