	...
	Do Until Rs1.EOF
		...
		%>
				<tr style="background-color:#f2f7fa">
					...
					<td class="bdr tit"><%=c1_code%></td>
					<td class="bdr tit"><span class="ui-icon ui-icon-folder-open" style="float:left"></span>
					...
					<%End If%></td>
					<td><input type="button" value="중분류추가" class="AXButtonSmall Red" onclick="location.href='?mode=reg&depth=2&p_code=<%=c1_code%>'"></td>
				</tr>
		<%
		'중분류
		...
		Do Until Rs2.EOF
			...
			%>
				<tr class="ct<%=c1_code%>" style="background-color:#faf7f2">
					<td class="bdr"></td>
					<td class="bdr tit"><%=c2_code%></td>
					<td class="bdr tit" style="padding-left:30px"><span class="ui-icon ui-icon-folder-open" style="float:left"></span>
					...
					<td><input type="button" value="소분류추가" class="AXButtonSmall Green" onclick="location.href='?mode=reg&depth=3&p_code=<%=c2_code%>'"></td>
				</tr>
			<%
			'소분류
			...
			Do Until Rs3.EOF
				...
				<tr class="ct<%=c2_code%>">
					<td class="bdr"></td>
					<td class="bdr tit"><%=c3_code%></td>
					<td class="bdr tit" style="padding-left:60px"><span class="ui-icon ui-icon-document" style="float:left"></span>
		...
		<div class="tb_list" style="width:49%; float:right; height:500px">
			...
			If mode="reg" Or mode="mod" Then
				...
				If mode="reg" Then
					...
					'언어정보 취득
					Sql = "SELECT * FROM tbl_cate WHERE c_code='" & p_code & "' "
					RsP.Open Sql, Dbcon, 1
					If RsP.EOf=False Then
						c_lang = RsP("c_lang")
					End If
				...
				Else
					...
					Set RsP = Server.Createobject("ADODB.Recordset")
					Sql = "SELECT * FROM tbl_cate WHERE c_code='" & c_code & "' "
					RsP.Open Sql, Dbcon, 1
					If RsP.EOf=False Then
						c_lang = RsP("c_lang")
						c_code = RsP("c_code")
