...
<div class="tb_list">
	...
	<form name="delform" action="?<%=pLink%>&amp;mode=seldel_p" method="post" enctype="multipart/form-data">
		<table>
			...
			If Rs.EOF=False Then
				...
				Do Until Rs.EOF Or cnt>=Rs.pagesize
					...
					<tr>
						...
						<td class="bdr bdt"><%=b_addtext1 & " ~ " & b_addtext2%></td>
						<td class="bdr bdt"><%=b_mem_name%></td>
						<td class="bdr bdt"><%=b_regdate%></td>
						<td class="bdr bdt"><%=b_read_count%></td>
						<td class="bdr bdt"><%
						If CDate(b_addtext1)>Date() Then
							Response.Write "<input type=""button"" value=""모집전"" class=""AXButtonSmall Classic"">"
						ElseIf CDate(b_addtext2)<Date() Then
							Response.Write "<input type=""button"" value=""마감"" class=""AXButtonSmall Classic"">"
						Else
							Response.Write "<input type=""button"" value=""모집중"" class=""AXButtonSmall Blue"">"
						End If
						%></td>
						<!-- <td class="bdr bdt"><%Call MovePost(bid, b_idx, b_num)%></td> -->
						<td class="bdt">
			...
			%>
				<tr>
					<td colspan="9" class="bdr bdt">No data</td>
				</tr>
			<%
			End If
			Rs.Close
