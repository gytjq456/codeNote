...
<div class="tb_info">
	...
	<table>
		...
		<%If c_addtext1<>"" Then%>
		<tr>
			<th class="title bdr bdt">Youtube 영상</th>
			<td class="bdt bdr">
			<!--유튜브 영상 -->
			<iframe src="https://www.youtube.com/embed/<%=c_addtext1%>" frameborder="0" allowfullscreen style="width:500px; height:350px"></iframe></td>
		</tr>
		<tr>
			<th class="title bdr bdt">Youtube 원본</th>
			<td class="bdt bdr"><a href="https://www.youtube.com/watch?v=<%=c_addtext1%>" target="_blank">https://www.youtube.com/watch?v=<%=c_addtext1%></a></td>
		</tr>
		<%End If%>
		<tr>
						...
						<form name="regform" action="?mode=comment_reg_p&amp;<%=pLink%>&amp;idx=<%=idx%>&amp;page=<%=page%>" method="post" enctype="multipart/form-data">
							...
							<p style="float:right">
								<%If k_spamcode=True Then%>
									<img src="/site/api/captcha/captcha.asp" id="imgCaptcha" style="height:35px" />
									<input type="button" value="새로고침" class="AXButton Classic" onclick="RefreshImage('imgCaptcha');" />
