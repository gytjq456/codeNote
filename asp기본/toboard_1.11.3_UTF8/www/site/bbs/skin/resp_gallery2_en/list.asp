		...
		Do Until Rs.EOF Or cnt>=Rs.pagesize
			...
			<li>
				...
				<%End If%>
					...
					<%Else%>
						<div class="img"><img src="http://placehold.it/380X380" alt="<%=b_idx%>" class="gallery-image" /></div>
					<%End If%>
					<div class="txt">
						<h4><%=TagText(b_title)%></h4>
						<div><%=strTagDel(b_content)%></div>
						<p><%=FormatDateCustom(b_regdate)%></p>
					</div>
				</a>
			</li>
			<%
...
<div class="ntb-listbtn-area">
	<%If CInt(a_write)>=CInt(W_LEVEL) Then%>
		<input type="button" value=" Write " class="AXButton Classic" onclick="location.href='?<%=pLink%>&amp;mode=reg'">
	<%End If%>
</div>
