		...
		If CInt(a_comment)>=CInt(W_LEVEL) Then%>
			<tr>
				<td colspan="2" class="left b_none">
					<%If W_ID="" Then%>
						이름 : <input type="text" name="name" size="15" maxlength="15" class="inp" />
					<%Else%>
						<%=W_NAME%> (<%=W_ID%>)
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<%If W_ID="" Then%>
						비밀번호 : <input type="password" name="pw" size="15" maxlength="15" class="inp" />
					<%Else%>
