<!-- 스마트에디터2 -->
<script type="text/javascript" src="<%=T_PATH%>/api/smarteditor2/js/HuskyEZCreator.js" charset="utf-8"></script>
<script>
$(function() {
	$("#addtext1").datepicker({dateFormat: 'yy-mm-dd'});
	$("#addtext2").datepicker({dateFormat: 'yy-mm-dd'});
});
</script>
...
<table class="ntb-tb-view" style="width:100%" cellpadding="0" cellspacing="0">
	...
	<tr>
		<th>기 간</th>
		<td class="left"><input type="text" name="addtext1" id="addtext1" maxlength="10" value="<%=c_addtext1%>" class="AXInput W100" autocomplete="off" maxlength="10" /> ~
		<input type="text" name="addtext2" id="addtext2" maxlength="10" value="<%=c_addtext2%>" class="AXInput W100" autocomplete="off" maxlength="10" /></td>
	</tr>
	<tr>
		<th>범 주</th>
		<td class="left">
			<label><span style="padding:4px; color:#fff; background-color:#dd4444"><input type="radio" name="addtext3" value="#dd4444" <%If Trim(c_addtext3)="#dd4444" Or c_addtext3="" Then Response.Write "checked"%>>#dd4444</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#ed862d"><input type="radio" name="addtext3" value="#ed862d" <%If Trim(c_addtext3)="#ed862d" Then Response.Write "checked"%>>#ed862d</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#bcb900"><input type="radio" name="addtext3" value="#bcb900" <%If Trim(c_addtext3)="#bcb900" Then Response.Write "checked"%>>#bcb900</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#23a849"><input type="radio" name="addtext3" value="#23a849" <%If Trim(c_addtext3)="#23a849" Then Response.Write "checked"%>>#23a849</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#00bca0"><input type="radio" name="addtext3" value="#00bca0" <%If Trim(c_addtext3)="#00bca0" Then Response.Write "checked"%>>#00bca0</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#2d76ed"><input type="radio" name="addtext3" value="#2d76ed" <%If Trim(c_addtext3)="#2d76ed" Then Response.Write "checked"%>>#2d76ed</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#a22bbc"><input type="radio" name="addtext3" value="#a22bbc" <%If Trim(c_addtext3)="#a22bbc" Then Response.Write "checked"%>>#a22bbc</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#d1177d"><input type="radio" name="addtext3" value="#d1177d" <%If Trim(c_addtext3)="#d1177d" Then Response.Write "checked"%>>#d1177d</span></label>
			<label><span style="padding:4px; color:#fff; background-color:#666666"><input type="radio" name="addtext3" value="#666666" <%If Trim(c_addtext3)="#666666" Then Response.Write "checked"%>>#666666</span></label></td>
	</tr>
	<tr>
		<th>제 목</th>
	...
	<tr>
		<th>자동등록방지</th>
		<td class="left">
		<img src="<%=T_PATH%>/api/captcha/captcha.asp" id="imgCaptcha" class="vmiddle" /> <input type="button" value="새로고침" class="AXButton Classic" onclick="RefreshImage('imgCaptcha');" />
...
function RegChk(privacy, spamcode) {
	...
	if(obj.addtext1.value==""){ alert('날짜를 입력하세요'); obj.addtext1.focus(); return; }
	if(obj.addtext2.value==""){ alert('날짜를 입력하세요'); obj.addtext2.focus(); return; }
...
nhn.husky.EZCreator.createInIFrame({
  ...
  sSkinURI: "<%=T_PATH%>/api/smarteditor2/SmartEditor2Skin.html",
