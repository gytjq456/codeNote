	...
	<table width="<%=T_WIDTH%>" cellpadding="0" cellspacing="0" class="ntb-tb-view">
		...
		Do Until Rs.EOF
			...
			<tr>
				...
				<img src="<%=T_PATH%>/images/board/icon_new.gif" class="vmiddle" alt="new" />
				<%End If%>
				</td>
				<td class="right b_none"><a href="?mode=comment_del_l&amp;<%=pLink%>&amp;idx=<%=idx%>&amp;com_idx=<%=bc_idx%>"><img src="<%=T_PATH%>/images/board/x.gif" class="vmiddle" alt="delete" /></a></td>
			</tr>
			<tr>
		...
		If CInt(a_comment)>=CInt(W_LEVEL) Then%>
			...
			<tr>
				...
				<td class="right"><input type="button" value=" 덧글 쓰기 " class="AXButton Classic" onclick="BoardCommentRegChk()"></td>
			</tr>
		<%End If%>
		</tbody>
	</table>
