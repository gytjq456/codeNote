...
<ul class="bbs-article">
	...
	<%Else%>
		...
		Do Until Rs.EOF Or cnt>=Rs.pagesize
			...
			b_title = Rs("title")
			b_content = Left(strTagDel(Rs("content")), 150)
			If Len(Rs("content"))>150 Then b_content=b_content & "..."
			...
			<li>
				<div class="thum-img"><%
					...
					<%Else%>
						<img src="http://placehold.it/198X138" alt="<%=b_idx%>" class="gallery-image" />
					<%End If%></a></div>
				<div class="cont">
					<p class="tit">
						<%If b_d_num<>"0" Then
						...
						<%End If%></p>
					<p class="date">
						<span class="sub_t">등록일 : </span> <span class="pdR20"><%=Left(b_regdate,10)%></span>
					</p>
